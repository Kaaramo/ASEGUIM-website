...
function ChiffreCard({ valeur, suffixe, titre, detail }: (typeof CHIFFRES)[number]) {
  ...
  return (
    <div ref={ref as React.RefObject<HTMLDivElement>} className="flex h-full flex-col justify-center rounded-3xl border border-cream/15 bg-cream/[0.04] p-8 text-center">
      ...
      <p className={`mt-1 text-sm text-cream/60 ${detail ? "" : "invisible"}`}>{detail || " "}</p>
    </div>
  );
}
      ...
      <div className="overflow-hidden rounded-[2rem] bg-ink">
        <div className="container-careloop section-y">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {CHIFFRES.map((c, i) => (
              <Reveal key={c.titre} delay={i * 90} className="h-full">
                <ChiffreCard {...c} />
              </Reveal>
            ))}
          </div>
